...
# Date: 2023-03-09
#
# Script Name: plot_target_cmflows
#
# Script Description: Given the target gene, plots sumary of cmflows
#
...
# combine regulons program gnes with symbol
regulons_programs_genes_symbols <- regulons_programs_genes |>
  left_join(identifiers, by= c("Gene" = "Preferred_Name")) |>
  filter(Source == "Gene Name") |>
  dplyr::rename("GeneSymbol" = "Name") |>
  dplyr::select(-Source) |>
  left_join(identifiers, by=c("Regulator" = "Preferred_Name")) |>
  filter(Source == "Gene Name") |>
...
plot_mutation_cmflows <- function(in_gene){
  ...
  # find regulons of target gene
  target_regulons <- regulons_programs_genes_symbols |>
    filter(GeneSymbol == in_gene) |>
    pull(Regulon_ID) |>
    unique()

  ## get CMflows with the mUtation as causal
  target_cmflows <- cmflows_disease_genes |>
    filter(Regulon %in% target_regulons) |>
